...
def move():
    ...
    for x in range(n):
        for y in range(n):
            # 상어가 존재하는 경우
            if array[x][y] != 0:
                direction = directions[array[x][y] - 1]  # 현재 상어 방향
                found = False
                # 일단 냄새가 존재하지 않는 곳이 있는지 확인
                for idx in range(4):
                    nx = x + dx[priorities[array[x][y] - 1][direction - 1][idx] - 1]
                    ny = y + dy[priorities[array[x][y] - 1][direction - 1][idx] - 1]
                    if 0 <= nx < n and 0 <= ny < n:
                        if smell[nx][ny][1] == 0:  # 냄새가 존재하지 않는 곳이면
                            # 해당 상어 방향 이동
                            directions[array[x][y] - 1] = priorities[array[x][y] - 1][direction - 1][idx]
                            # 만약 이미 다른 상어가 있다면 번호가 낮은 상어가 들어가도록 상어 이동
                ...
                for idx in range(4):
                    ...
                    ny = y + dy[priorities[array[x][y] - 1][direction - 1][idx] - 1]
                    if 0 <= nx < n and 0 <= ny < n:
                        if smell[nx][ny][0] == array[x][y]: # 자신의 냄새가 있는곳
                            # 해당 상어 방향 이동
                            directions[array[x][y] - 1] = priorities[array[x][y]-1][direction-1][idx]
                            # 상어 이동
                            new_array[nx][ny] = array[x][y]
